 ...
 * @author  spuhl
 */
public class WorkbenchWidget extends SearchResultWidget implements TreeSelectionListener, FeatureSelectionChangedListener, PropertyChangeListener, FeatureCollectionListener {
...
//    private final DefaultMutableTreeTableNode processedObjects = new DefaultMutableTreeTableNode(null, true);
    ...
    private HashMap<Standort, ArrayList<Leuchte>> leuchtenRemovedFromMastMap = new HashMap<Standort, ArrayList<Leuchte>>();
...
//    }
    ...
    private final Set removedObjects = new TreeSet(new ReverseComparator(new EntityComparator(new ReverseComparator(new LeuchteComparator()))));
     ...
     */
    public void setSelectedTreeNode(TreePath selectedTreeNode) {
        TreePath oldSelectedTreeNode = this.selectedTreeNode;
...
//        });
        ...
        final HighlightPredicate noGeometryPredicate = new HighlightPredicate() {
            ...
            public boolean isHighlighted(Component renderer, ComponentAdapter componentAdapter) {
                ...
                try {
                    Object userObj = ((AbstractMutableTreeTableNode) jttHitTable.getPathForRow(componentAdapter.row).getLastPathComponent()).getUserObject();
                    if (userObj != null) {
                        ...
                        if (userObj instanceof GeoBaseEntity) {
                            return ((GeoBaseEntity) userObj).getGeometry() == null;
                                ...
                                (virtualStandort = leuchteToVirtualStandortMap.get((Leuchte) userObj)) != null) {
        ...
        Toolkit.getDefaultToolkit().addAWTEventListener(new AWTEventListener() {
            ...
            public void eventDispatched(AWTEvent event) {
                if (((BelisBroker) broker).isVetoCheckEnabled() && (((BelisBroker) broker).isInCreateMode() || ((BelisBroker) broker).isInEditMode()) && ((MouseEvent) event).getSource() instanceof JXTreeTable &&
                        (((MouseEvent) event).getID() == MouseEvent.MOUSE_PRESSED || ((MouseEvent) event).getID() == MouseEvent.MOUSE_CLICKED || ((MouseEvent) event).getID() == MouseEvent.MOUSE_RELEASED || ((MouseEvent) event).getID() == MouseEvent.MOUSE_DRAGGED)) {
                    ...
                    if (clickedRow != selectedRow) {
                        ...
                        log.debug("eventDispatched: Not consuming event. No Selection changed would be happen.");
                    }
                }
            }
        ...
        Toolkit.getDefaultToolkit().addAWTEventListener(new AWTEventListener() {
            ...
            @Override
            public void eventDispatched(AWTEvent event) {
                if (((BelisBroker) broker).isVetoCheckEnabled() && (((BelisBroker) broker).isInCreateMode() || ((BelisBroker) broker).isInEditMode()) && ((KeyEvent) event).getSource() instanceof JXTreeTable &&
                        (((KeyEvent) event).getID() == KeyEvent.KEY_PRESSED || ((KeyEvent) event).getID() == KeyEvent.KEY_RELEASED || ((KeyEvent) event).getID() == KeyEvent.KEY_TYPED)) {
    ...
    public void selectNode(CustomMutableTreeTableNode nodeToSelect) {
...
//    private void tryToChangeVetoCheckEnabled(boolean isEnabled){
////        if(!isEnabled){
////            log.debug("disable veto mode");
////            if(!((BelisBroker)broker).isVetoCheckEnabled()){
////                log.debug("vetomode already disabled");
...
////                //((BelisBroker)broker).setVetoCheckEnabled(false);
////            } else {
////                log.debug("disabling vetomode");
////                //ToDo it would be cool to make belis widegts for the core widgets so no dump casting.
////               ((BelisBroker)broker).setVetoCheckEnabled(false);
////            }
////        } else {
////            if(isAlreadyDisabled){
////                log.info("not enabling veto check because there was an action before which disabled it");
////            } else {
////                log.debug("reanabling vetocheck and reseting flag");
////                ((BelisBroker)broker).setVetoCheckEnabled(true);
...
//    }
                    ...
                    if (userObject != null) {
                        ...
                        if (userObject instanceof GeoBaseEntity) {
                            ...
                            if (((GeoBaseEntity) userObject).getGeometry() == null) {
                                ...
                                (virtualStandort = leuchteToVirtualStandortMap.get((Leuchte) userObject)) != null &&
    ...
    //ToDo if application switches in Edit Mode it must be checked if there is a entry selected and the mode must be switched
    @Override
    public void valueChanged(TreeSelectionEvent e) {
        log.debug("JTreeTable selection changend: " + e.getPaths());
        try {
            try {
                log.debug("DetailWidget is valid: " + (Validatable.VALID == ((BelisBroker) broker).getDetailWidget().getStatus()));
            } catch (Exception ex) {
            ...
            if (e.isAddedPath()) {
                ...
                if (isSelectedOverMap) {
                    ...
                    if (currentUserObject != null && currentUserObject instanceof StyledFeature && ((StyledFeature) currentUserObject).getGeometry() != null) {
                        ...
                        Standort parentMast = getParentMast(e.getPath().getLastPathComponent());
                        if (broker.getMappingComponent().getFeatureCollection().getSelectedFeatures() != null &&
                                broker.getMappingComponent().getFeatureCollection().getSelectedFeatures().size() == 1 &&
                                broker.getMappingComponent().getFeatureCollection().getSelectedFeatures().iterator().next().equals(parentMast)) {
                        ...
                        log.debug("current selected node is haengeleuchte. Selecting corresponding standort in map: ");
    ...
    public boolean isNodeHaengeLeuchte(Object node) {
        try {
            log.debug("instance of Leuchte: " + (((CustomMutableTreeTableNode) node).getUserObject() instanceof Leuchte));
            log.debug("Parent is searchnode: " + ((CustomMutableTreeTableNode) ((CustomMutableTreeTableNode) node).getParent()).equals(searchResultsNode));
            return ((CustomMutableTreeTableNode) node).getUserObject() instanceof Leuchte &&
                    (((CustomMutableTreeTableNode) ((CustomMutableTreeTableNode) node).getParent()).equals(searchResultsNode) ||
                    ((CustomMutableTreeTableNode) ((CustomMutableTreeTableNode) node).getParent()).equals(newObjectsNode));
        } catch (Exception ex) {
    ...
    public Standort getParentMast(Object node) {
        try {
            return ((Standort) ((CustomMutableTreeTableNode) ((CustomMutableTreeTableNode) node).getParent()).getUserObject());
        } catch (Exception ex) {
        ...
        if (!ignoreFeatureSelection) {
            ...
            try {
                ...
                for (Feature feature : features) {
                    ...
                    if (feature instanceof GeoBaseEntity && broker.getMappingComponent().getFeatureCollection().isSelected(feature)) {
...
//                }
                        ...
                        if (feature instanceof Standort && !((Standort) feature).isStandortMast() &&
                            ...
                            path = treeTableModel.getPathForUserObject(((Standort) feature).getLeuchten().iterator().next());
                        } else {
    ...
    public void refreshTreeArtifacts(int artifactType) {
...
//                    }
                ...
                if (searchResultsNode != null) {
                    for (Object curObject : currentSearchResults) {
                        if (curObject instanceof Standort) {
                            ...
                            if (searchResultsNode != null) {
                                if (((Standort) curObject).isStandortMast()) {
                                    ...
                                    log.debug("Current Object is standort for Hängeleuchte (virtual) not adding to tree");
                                }
                                if (leuchten != null) {
...
//                }
            ...
            case REFRESH_ALL:
                ...
                break;
            //ToDo disabled Functionality 04.05.2009
            //ToDo is not realy an refresh is more a special function
//            case REFRESH_PROCESSED_OBJECTS:
//                log.debug("Refreshing processed objects");
//                if (newObjectsNode.getChildCount() > 0) {
//                    log.debug("Child index count: " + newObjectsNode.getChildCount());
//                    final int childIndexCount = newObjectsNode.getChildCount();
//                    for (int curChildIndex = 0; curChildIndex < childIndexCount; curChildIndex++) {
//                        final MutableTreeTableNode curChild = (MutableTreeTableNode) newObjectsNode.getChildAt(0);
//                        removeNodeFromParent(curChild);
//                        log.debug("check if node is placeholder node");
//                        CustomMutableTreeTableNode realStandortNode = null;
//                        if ((realStandortNode = leuchtePlaceholderNodeToActualStandort.get(curChild)) != null) {
//                            log.debug("current node is placeholdernode. Creating placeholderStandort");
//                            //Problem --> what about the leuchten wich are not processed is it ok + what if the standort is changed and a leuchte is added attention
//                            final CustomMutableTreeTableNode clonedStandortNode = cloneNodeWithSameUserObjects(realStandortNode);
//                            insertNodeInto(clonedStandortNode, processedObjectsNode, processedObjectsNode.getChildCount());
//                        } else {
//                            log.debug("current node is no placeholdernode.");
//                            insertNodeInto(curChild, processedObjectsNode, processedObjectsNode.getChildCount());
//                        }
//                    }
//                    processedObjects.add(newObjects);
//                    newObjects.clear();
//                } else {
//                    log.debug("no new objects to move to processed objects");
//                }
//                return;
//            case REFRESH_NEW_OBJECTS:
    ...
    public void removeEntity(Object entity) {
        ...
        if (entity != null) {
            ...
            if (pathToNodeToRemove != null && pathToNodeToRemove.getLastPathComponent() != null &&
                ...
                final CustomMutableTreeTableNode nodeToRemove = (CustomMutableTreeTableNode) pathToNodeToRemove.getLastPathComponent();
                saveSelectedElementAndUnselectAll();
                ...
                if (nodeToRemove.getUserObject() != null &&
                    ...
                    if (mast.getLeuchten().size() == 0) {
                        ...
                        broker.getMappingComponent().getFeatureCollection().reconsiderFeature(((Standort) ((CustomMutableTreeTableNode) nodeToRemove.getParent()).getUserObject()));
                    }
                    ...
                    final Standort virtualStandort = leuchteToVirtualStandortMap.get((Leuchte) nodeToRemove.getUserObject());
...
//    public Set getSavedObjects() {
//        return savedObjects;
//    }
//
//    public void setSavedObjects(Set savedObjects) {
//        log.debug("setSavedObjects");
...
//        //refreshTreeArtifacts(MOVE_NEW_TO_SAVED_OBJECTS);
//
//
//        //ToDo ugly
//        //remove and add to map
////        for (Object curObject : newObjects) {
////            broker.getMappingComponent().getFeatureCollection().removeFeature((Feature) curObject);
////        }
//        broker.getMappingComponent().getFeatureCollection().removeFeatures(newObjects);
//        newObjects.clear();
//        if (newlySaved != null) {
//            savedObjects.addAll(newlySaved);
////            for (Object curObject : newlySaved) {
...
//    }
    public void restoreRemovedObjects() {
        ...
        if (leuchtenRemovedFromMastMap.keySet().size() > 0) {
            ...
            for (Standort curMast : leuchtenRemovedFromMastMap.keySet()) {
    ...
    public void objectsRemoved() {
        ...
        if (!((BelisBroker) broker).isInCreateMode()) {
            if (removedObjects.size() > 0) {
                for (Object curObjectToRemove : removedObjects) {
...
//    public void refreshSavedObjects(Set refreshedSaved){
//        if(refreshedSaved != null){
//            savedObjects.cle
//        }
//    }
    ...
    public CustomMutableTreeTableNode addNewLeuchte(Object relatedObject) {
        CustomMutableTreeTableNode nodeToAddLeuchte = null;
        Standort parent = null;
        try {
            TreePath pathToRelatedObejct = treeTableModel.getPathForUserObject(relatedObject);
            if (pathToRelatedObejct != null && (pathToRelatedObejct.getLastPathComponent() != null &&
                    pathToRelatedObejct.getLastPathComponent() instanceof CustomMutableTreeTableNode &&
                    ((CustomMutableTreeTableNode) pathToRelatedObejct.getLastPathComponent()).getUserObject() instanceof Leuchte) &&
                    pathToRelatedObejct.getParentPath() != null &&
                    pathToRelatedObejct.getParentPath().getLastPathComponent() instanceof CustomMutableTreeTableNode &&
                    ((CustomMutableTreeTableNode) pathToRelatedObejct.getParentPath().getLastPathComponent()).getUserObject() instanceof Standort) {
                parent = (Standort) ((CustomMutableTreeTableNode) pathToRelatedObejct.getParentPath().getLastPathComponent()).getUserObject();
                nodeToAddLeuchte = ((CustomMutableTreeTableNode) pathToRelatedObejct.getParentPath().getLastPathComponent());
                log.debug("Related object is Leuchte (sibling)");
            } else if (pathToRelatedObejct != null && pathToRelatedObejct.getLastPathComponent() != null && pathToRelatedObejct.getLastPathComponent() instanceof CustomMutableTreeTableNode &&
                    ((CustomMutableTreeTableNode) pathToRelatedObejct.getLastPathComponent()).getUserObject() instanceof Standort) {
                log.debug("Related object is Standort (parent)");
                parent = (Standort) ((CustomMutableTreeTableNode) pathToRelatedObejct.getLastPathComponent()).getUserObject();
                nodeToAddLeuchte = ((CustomMutableTreeTableNode) pathToRelatedObejct.getLastPathComponent());
            } else if (pathToRelatedObejct == null && relatedObject != null && relatedObject instanceof Standort && (((Standort) relatedObject).isVirtuellerStandort() != null && ((Standort) relatedObject).isVirtuellerStandort()) || !((Standort) relatedObject).isStandortMast()) {
        ...
        Leuchte newLeuchte = new Leuchte();
...
//        }
    ...
    public short getNextLeuchtennummer(Standort standort) {
        if (standort != null && standort.getLeuchten() != null && standort.getLeuchten().size() > 0) {
            short max = 0;
            for (Leuchte curLeuchte : standort.getLeuchten()) {
    ...
    //ToDo Workaround because there will be exceptions if a selectedNode is moved. Should normaly not be done in the model
